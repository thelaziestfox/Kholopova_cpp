...
RandIt getMax(int n, RandIt start, RandIt finish)
{
    ...
    std::vector<typename std::iterator_traits<RandIt>::value_type> maxs(n);
    std::vector<std::thread> threads;
    for (int i = 0; i < n; ++i) {
        threads.emplace_back([&, i]()
        {
            RandIt begin = start + i * length / n;
            RandIt end = i == n - 1 ? finish : start + (i + 1) * length / n;
            maxs[i] = *std::max_element(begin, end);
        });
    }
    for (auto& t : threads)
    {
